...
namespace Spawn
{
    public class RandomShipSpace
    {

        /*  
         *  1. Create a folder (This can be anywhere you want) and add any Ship.sbc's to it that you wish to be included in the random selection and set the `RandomShipFolder`
         *  string to the full path of said folder.
         *
         *  2. Create a placeholder sbc in your `SpawnShips` folder (e.g. `Random.sbc`) that will be used to spawn the randomly selected ship, you will set the PrefabName to 
         *  whatever you name this file and you need to set `RandomShipSBC` to its file path.
         *
         *
         *
         *
        ...
        private static Vector3D RegionCenter = new Vector3D(0, 0, 0); //This is XYZ coords of the region center. This can be pulled from Nexus Controller Sectorsetup.
        private static double RegionRadius = 250; //Radius is in KM. This also can be pulled from Controller SectorSetup
        ...
        private static string RandomShipFolder = @"C:\Services\Space Engineers\SpawnPads\SpawnShips\Random"; //This directory will contain the grids you want to be included in the randomization.
        private static string RandomShipSBC = @"C:\Services\Space Engineers\SpawnPads\SpawnShips\Random.sbc"; //This file will be overwritten whenever a new random grid is picked and will also be the Prefab you set in the spawn pads custom data.
        ...
        public void BeforeSpawn(ref List<MyObjectBuilder_CubeGrid> Grids, ref List<long> Players, ref Vector3D SpawnPos, ref bool AlignToGravity, string CustomData = null)
        {
            ...
            string[] GridArray = Directory.GetFiles(RandomShipFolder + "\\"); //Pull's all grid paths in selected folder.
            Random rnd = new Random();
            
            string SelectedGrid = File.ReadAllText(GridArray[rnd.Next(GridArray.Length)]); //Selects a random grid from the list.
            File.WriteAllText(RandomShipSBC, SelectedGrid); //Replaces Random.sbc with the randomly selected grid from the list.
